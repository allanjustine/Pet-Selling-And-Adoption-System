...
@section('title', "$app_name | Edit Pet")
            ...
            <div class="col-12 col-md-8 order-last order-md-first">
                <div class="card">
                    <div class="card-body">
                        ...
                        <form action="{{ route('seller.pets.update', $pet) }}" method="post" enctype="multipart/form-data"
                            id="pet_form">
                            @csrf @method('PUT')
                                ...
                                <select class="form-control" name="category_id" required onchange="showInputs(this)">
                                    ...
                                    @foreach ($categories as $category)
                                        <option value="{{ $category->id }}"
                                            @if ($pet->category_id == $category->id) selected @endif
                            ...
                            <div class="form-group mb-3">
                                ...
                                <select class="form-control form-control-sm" name="breed_id" required>
                                    ...
                                    @foreach ($breeds as $id => $breed)
                                        <option value="{{ $id }}"
                                            @if ($pet->breed_id == $id) selected @endif>
                                            {{ $breed }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group mb-3">
                                ...
                                <select class="form-control form-control-sm" name="type">
                                    <option value=""></option>
                                    <option value="pure breed" @if ($pet->type == 'pure breed') selected @endif>
                                        Pure Breed
                                    </option>
                                    <option value="cross breed" @if ($pet->type == 'cross breed') selected @endif>
                                        Cross Breed
                                    </option>
                                </select>
                            </div>
                            ...
                            <div class="{{ !$pet->category->has_vaccination ? 'd-none' : '' }}"
                            ...
                            <div class="{{ !$pet->category->has_deworming ? 'd-none' : '' }}"
                            ...
                            {{-- <div class="form-group mb-3">
                                <label class="form-label">Vaccine Taken (Comma Separated) *</label>
                                <input type="text" class="form-control form-control-sm" name="vaccine_taken"
                                    placeholder="Eg. 3 in 1,5 in 1" value="{{ $pet->vaccine_taken }}" required>
                            </div> --}}

                            <div class="form-group mb-3">
                                <label class="form-label">Price (₱) *</label>
                                <input type="number" min="0" class="form-control form-control-sm" name="price"
                                    placeholder="eg. ₱1500" value="{{ $pet->price }}" required>
                            </div>
                            ...
                            <div class="form-group">
                                <button type="button" class="btn btn-primary btn-sm w-100"
                                    onclick="promptUpdate(event, '#pet_form', 'Do you want to Update?', 'Note: If you upload a new set of images it will overwrite the existing one.', 'Yes')">Save</button>
                            </div>
                        </form>
                    </div>
                </div>
            ...
            <div class="col-md-4 order-first order-md-last">
                <div class="card">
                    <div class="card-body text-center">
                        ...
                        <br>
                        <h4 class="font-weight-normal">Name: {{ $pet->name }}</h4>
                        <h4 class="font-weight-normal">Breed: {{ $pet->breed->name }}</h4>
                        <h4 class="font-weight-normal">Breed: {{ $pet->type }}</h4>
                        ...
                        <div>
                            ...
                            <a href="{{ handleNullImage($pet->proof_of_ownership) }}" class="glightbox">
                                <img class="img-fluid" src="{{ handleNullImage($pet->proof_of_ownership) }}"
