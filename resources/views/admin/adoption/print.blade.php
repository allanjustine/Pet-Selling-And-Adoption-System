...
@section('content')
    <div class="container mt-5">
        ...
        <br>
        <h3 class="text-center">Pet for Adoption Record</h3> <br>
            ...
            <tbody>
                ...
                @forelse ($adoptions as $adoption)
                    <tr>
                        ...
                        <td>{{ $adoption->breed->name }}</td>
                        <td>{{ $adoption->category->name }}</td>
                        <td>{{ $adoption->user->full_name }}</td>
                        <td>{{ $adoption->is_adopted ? 'Adopted' : 'Open for Adoption' }}</td>
                        <td>{!! strip_tags(isApproved($adoption->status)) !!}</td>
                        <td>{{ formatDate($adoption->created_at, 'default') }}</td>
                    </tr>
                @empty
                    <tr>
    ...
    <script>
        ...
        onafterprint = function() {
            window.location.href = @json(route('admin.adoptions.index'));
        }
    </script>
@endsection
