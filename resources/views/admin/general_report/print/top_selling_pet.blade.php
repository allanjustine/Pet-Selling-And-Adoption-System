...
@section('content')
    <div class="container mt-5">
        ...
        <br>
        <h3 class="text-center">Top Selling Pet</h3> <br>
        <form class="d-print-none" action="{{ route('admin.print.handle') }}" method="GET">
            <div class="input-group">
                <input type="hidden" name="records" value="top_selling_pet">
        ...
        <table class="table table-flush table-hover">
            ...
            <tbody>
                @forelse (array_combine($top_selling_pet[0], $top_selling_pet[1]) as $pet => $total_order)
                    <tr>
                        <td>{{ $pet }}</td>
                        <td>{{ $total_order }}</td>
                    </tr>
