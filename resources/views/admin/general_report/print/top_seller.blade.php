...
@section('content')
    <div class="container mt-5">
        ...
        <br>
        <h3 class="text-center">Top Seller</h3> <br>
        <form class="d-print-none" action="{{ route('admin.print.handle') }}" method="GET">
            <div class="input-group">
                <input type="hidden" name="records" value="top_seller">
        ...
        <table class="table table-flush table-hover">
            ...
            <tbody>
                @forelse (array_combine($top_seller[0], $top_seller[1]) as $seller => $total_order)
                    <tr>
                        <td>{{ $seller }}</td>
                        <td>{{ $total_order }}</td>
                    </tr>
